    ...
    export class JourneyVis extends DataVis {
        ...
        #initVis() {
            ...
            this.el.vis.group.append('rect')
                ...
                .attr('y', margin.top * 0)
                .attr('width', width -   0 * margin.left - 0 * margin.right)
                .attr('height', height - 0 * margin.top  - 0 * margin.bottom)
        ...
        #clearDynamic() {
            ...
            this.el.vis.recoveryRate.selectAll('.sparkline').remove()
            this.el.annotation.group.selectAll('.volume-change-annotation').remove()
            this.el.annotation.group.selectAll('.recovery-rate-annotation').remove()
        }
        ...
        #prepareRidgeSeries({ metric, groupBy = 'stream', index = true, sort = 'lastValue', resolver = null, perCapita = false }) {
            ...
            const series = groups.map(group => {
                const raw = this.#getActiveYears().map(({ year }) => {
        ...
        #prepareSparklineSeries({ metric, groupBy = 'stream', sort = 'none' }) {
            ...
            const series = groups.map(group => ({
                label:  sectorLabel[group] ?? group,
                values: this.#getActiveYears().map(({ year }) => ({
                    year,
        ...
        #drawRidgelinePlot(series, config = {}) {
            ...
            const duration = this.state.animation?.duration ?? 800

            // -- Add ridgeline group
            const root = this.el.vis.volumeChange 
                .append('g')
                .classed(`ridgeline-group ${cfg.classType}`, true)
            ...
            if (cfg.title) {
                root.append('text')
                    .classed(`title ${cfg.classType}`, true)
                    .attr('x', 4)
                    .attr('y', 0)
            ...
            if (cfg.showAxis && cfg.axisOnTop) {
                root.append('g')
                    .classed(`ridge-axis-x ${cfg.classType}`, true)
            ...
            series.forEach((s, i) => {
                ...
                // Flat line at mean value — starting state for animation
                const meanVal   = d3.mean(s.values, d => d.value)
                const flatLine  = d3.line()
                    .x(d => this.scale.ridgeX(d.year))
                    .y(() => this.scale.ridgeY(meanVal))
                    .curve(cfg.curve)

                const flatArea  = d3.area()
                    .x(d  => this.scale.ridgeX(d.year))
                    .y0(bottomY)
                    .y1(() => this.scale.ridgeY(meanVal))
                    .curve(cfg.curve)
                ...
                const rg = root.append('g')
                    .classed(`ridge ${cfg.seriesType} ${cfg.classType} ${sc}`, true)
                    .attr('transform', `translate(0, ${yOff})`)

                // vi. Area: animate from flat to final
                rg.append('path')
                    .datum(s.values)
                    .classed(`ridge-area ${cfg.seriesType} ${cfg.classType} ${sc}`, true)
                    .attr('d', flatArea)
                    .transition()
                    .duration(duration)
                ...
                // Line: animate from flat to final
                rg.append('path')
                    .datum(s.values)
                    .classed(`ridge-line ${cfg.seriesType} ${cfg.classType} ${sc}`, true)
                    .attr('d', flatLine)
                ...
                const labelEl = rg.append('text')
                    .classed(`ridge-label start ${cfg.seriesType} ${cfg.classType} ${sc}`, true)
                    .attr('x', -fontSize)
                ...
                rg.append('text')
                    .classed(`ridge-label ridge-label-end ${cfg.seriesType} ${cfg.classType} ${sc}`, true)
                    .attr('x', plotW + fontSize * 0.35)
        ...
        #drawSparkline(series, cell, config = {}) {
            ...
            const duration = this.state.animation?.duration  * 4 ?? 2000
            ...
            // Flat area at y0 = plotH, y1 = plotH (zero height, grows up)
            const flatArea = d3.area()
                .x(d => xScale(d.year))
                .y0(plotH)
                .y1(plotH)
        ...
        #createSparkGradient(id, y, stops) {
        ...
        #addVolumeTitles(rowY){
            ...
            titleG.append('text')
                ...
                .text(`The contours of Victorian waste volumes`)

            titleG.append('text')
                .classed('volume-change subtitle generated', true)
                .attr('x', canvasWidth * 0.25)
                .attr('y', canvasHeight * 0.06)
                .style('font-size', subtitleFs)
                .text('generated')

            titleG.append('text')
                .classed('volume-change subtitle recovered', true)
        ...
        #addRecoveryRateTitles(rowY){
            ...
            const row1Y = canvasHeight * 0.095
            const row2Y = canvasHeight * 0.235
            ...
            titleG.append('text')
                ...
                .style('font-size', titleFs)
                .text(`The patterns of waste recovery rates`)
        ...
        // Helpers
        #getActiveYears() {
            return this.app.module.dataModel.getYears()
                .filter(({ year }) => year <= this.app.state.select.year)
        }
        ...
        update() {
            const { dataModel } = this.app.module
            if (!dataModel.data[this.app.state.select.year]) {
                return console.warn(`No data for year ${this.app.state.select.year}`)
            }
            this.#clearDynamic()
            this.render()
        }
        ...
        render() {
            ...
            const rowY = { 
                A:    margin.top + canvasHeight * 0.075, 
                B:    margin.top + canvasHeight * 0.145, 
                C:    margin.top + canvasHeight * 0.265, 
                D:    margin.top + canvasHeight * 0.55, 
            }
            ...
            this.#addRidglineBG(colX, rowY, colW)
            ...
            this.#drawRidgelinePlot(generatedTotal, {
                ...
                classType:      'generated',        labelMap:       { 'Total': 'Victoria' },
            })
            ...
            this.#drawRidgelinePlot(generatedStream, {
                ...
                classType:      'generated',        showAxis:       true
            })
            ...
            this.#drawRidgelinePlot(recoveredTotal, {
                ...
                classType:      'recovered',        labelMap:       { 'Total': 'Victoria' },
            })
            ...
            this.#drawRidgelinePlot(recoveredStream, {
                ...
                classType:      'recovered',        showAxis:       true
            })
            ...
            const sparkCells1 = this.#makeGrid(grid1Series.length, {
                x:       gridConfig.xPos ,
                y:       margin.top + canvasHeight * 0.665,
            ...
            const streamCells1 = this.#makeGrid(grid1Series.length, {
                x:       gridConfig.xPos ,
                y:       margin.top + canvasHeight * 0.8,
                width:   gridConfig.width,
                height:  gridConfig.rowHeight,
                cols:    gridConfig.noCols,
                padX:    gridConfig.padX,
            ...
            const streamCells2 = this.#makeGrid(grid1Series.length, {
                x:       gridConfig.xPos ,
                y:       margin.top + canvasHeight * 0.92,
                width:   gridConfig.width,
                height:  gridConfig.rowHeight,
                cols:    gridConfig.noCols,
                padX:    gridConfig.padX,
            ...
            const gradientStops = [
                { offset: '0%',    color: 'var(--recover-rate-0' },   
                { offset: '70%',  color: 'var(--recover-rate-100)' },
            ]
                        
            const gradient = this.#createSparkGradient('sparkline-gradient-1', gridConfig.rowHeight, gradientStops)
            ...
            streamSpark.series.slice(0,4).forEach((s, i) => {
                this.#drawSparkline(s, streamCells1[i], {
                    yDomain:        sparkYDomain,
                    showValues:     true,
                    showDates:      false,
                    formatValue:    v => Math.round(v) + '%',
            ...
            streamSpark.series.slice(4,8).forEach((s, i) => {
                this.#drawSparkline(s, streamCells2[i], {
                    yDomain:        sparkYDomain,
                    showValues:     true,
                    showDates:      false,
                    formatValue:    v => Math.round(v) + '%',
